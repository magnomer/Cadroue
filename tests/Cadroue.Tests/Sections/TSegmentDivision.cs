...
public sealed class TSegmentDivision
{
    ...
    public void CursorInsideActiveSection_DividesIntoTwoHalves()
    {
        ...
        Assert.Equal(TSegmentAtCreate(0), plan!.LPieceSections[0].LPieceOrigin);
        Assert.Equal(TSegmentAtCreate(3), plan!.LPieceSections[0].LPieceEnd);
        Assert.Equal(TSegmentAtCreate(3), plan!.LPieceSections[1].LPieceOrigin);
        Assert.Equal(TSegmentAtCreate(8), plan!.LPieceSections[1].LPieceEnd);
        Assert.Equal(0, plan!.LPieceFirst);
        Assert.Equal(1, plan!.LPieceSecond);
    }
