...
public sealed class SectionValidationTests
{
    ...
    public void OverlapAllowed_InsideCheckIsFalse()
    {
        ...
        Assert.False(TInterface.PieceInsideCheck(sections, At(5), -1, OverlapOn));
    }
    ...
    public void TimeStrictlyInsideSection_IsInside()
    {
        ...
        Assert.True(TInterface.PieceInsideCheck(sections, At(5), -1, OverlapOff));
    }
    ...
    {
        var sections = new[] { Seg(2, 8) };
        Assert.False(TInterface.PieceInsideCheck(sections, At(8), -1, OverlapOff));
    }
    ...
    {
        var sections = new[] { Seg(2, 8) };
        Assert.True(TInterface.PieceInsideCheck(sections, At(2), -1, OverlapOff));
    }
    ...
    {
        var sections = new[] { Seg(2, 8) };
        Assert.False(TInterface.PieceInsideCheck(sections, At(5), 0, OverlapOff));
    }
    ...
    public void OverlapAllowed_LimitIsCeiling()
    {
        ...
        Assert.Equal(At(10), TInterface.PieceLimitRead(sections, At(0), At(10), -1, OverlapOn));
    }
    ...
    {
        var sections = new[] { Seg(5, 8) };
        Assert.Equal(At(5), TInterface.PieceLimitRead(sections, At(0), At(10), -1, OverlapOff));
    }
    ...
    {
        var sections = new[] { Seg(5, 8) };
        Assert.Equal(At(5), TInterface.PieceLimitRead(sections, At(0), At(5), -1, OverlapOff));
    }

    [Fact]
    public void EmptySections_FloorIsZero()
    {
        Assert.Equal(TimeSpan.Zero, TInterface.PieceFloorRead(Array.Empty<LPiece>(), At(5), -1, OverlapOff));
    }
    ...
    public void PriorSectionNearestCursor_SetsFloor()
    {
        ...
        Assert.Equal(At(4), TInterface.PieceFloorRead(sections, At(6), -1, OverlapOff));
    }
    ...
    public void OverlapAllowed_FloorIsZero()
    {
        ...
        Assert.Equal(TimeSpan.Zero, TInterface.PieceFloorRead(sections, At(6), -1, OverlapOn));
    }
}
