...
public sealed class TWorkNormalize
{
    private static LWorkNormalizeStep TWorkNormalizeCreate(
        double target = -21, double peak = -2, double range = 6,
        double frame = 300, double gauss = 21, double maxGain = 10, double compress = 6) =>
        (LWorkNormalizeStep)TInterface.TWorkNormalizeCreate(
    ...
    {
        LWorkNormalizeStep step = TWorkNormalizeCreate(
