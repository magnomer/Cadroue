...
public sealed class TTraceFormatting
{
    ...
    public void FirstAndLaterEntries_HaveProductionDeltaAndChronologicalTime()
    {
        ...
        TimeOnly first = TimeOnly.ParseExact(
            logging.TTraceEntries[0].TTraceTime,
            "HH:mm:ss.fff",
            CultureInfo.InvariantCulture);
        TimeOnly second = TimeOnly.ParseExact(
            logging.TTraceEntries[1].TTraceTime,
            "HH:mm:ss.fff",
            CultureInfo.InvariantCulture);
    ...
    public void LoggingOneCategory_DoesNotMutateAnotherCategory()
    {
        ...
        Assert.Equal(("Warning", "warning text", "warning detail"),
            (logging.TTraceEntries[0].TTraceKind,
                logging.TTraceEntries[0].TTraceSummary,
                logging.TTraceEntries[0].TTraceDetail));
        Assert.Equal(("Info", "information text", "information detail"),
            (logging.TTraceEntries[1].TTraceKind,
                logging.TTraceEntries[1].TTraceSummary,
                logging.TTraceEntries[1].TTraceDetail));
    }
