...
public sealed class TMediaProbe
{
    [Fact]
    public void ProbeProcess_UsesUtf8AndOmitsUnneededMetadata()
    {
        ProcessStartInfo startInfo = TScout.TScoutProbeCreate("한글 제목.mp4");
    ...
    {
        ProcessStartInfo startInfo = TScout.TScoutStreamCreate("한글 제목.mp4");
    ...
    {
        TimeSpan? end = TScout.TScoutEndParse(
            "9600.000000\n9599.933267\n9600.033367\nN/A\n",
    ...
    {
        Assert.ThrowsAny<Exception>(() => TScout.TScoutProbeParse(output));
    }

    internal static string TMediaProbeRead(string duration, params string[] streams) =>
        $$"""
