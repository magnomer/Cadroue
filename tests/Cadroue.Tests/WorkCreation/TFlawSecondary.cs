...
public sealed class TFlawSecondary
{
    private const string TFlawSubtitle =
        "[STREAM]\ncodec_type=video\ncodec_name=h264\n[/STREAM]\n"
        + "[STREAM]\ncodec_type=subtitle\ncodec_name=subrip\n[/STREAM]\n";

    private const string TFlawVideoOnly =
        "[STREAM]\ncodec_type=video\ncodec_name=h264\n[/STREAM]\n"
        + "[STREAM]\ncodec_type=audio\ncodec_name=aac\n[/STREAM]\n";
    ...
    {
        Assert.Null(TInterface.TFlawSecondaryResolve(TFlawVideoOnly, string.Empty, string.Empty));
    }

    [Fact]
    public void CleanSecondaryStream_ProducesNoDossier()
    {
        Assert.Null(TInterface.TFlawSecondaryResolve(TFlawSubtitle, string.Empty, string.Empty));
    }
    ...
    {
        LDossier? dossier = TInterface.TFlawSecondaryResolve(
            TFlawSubtitle,
            string.Empty,
