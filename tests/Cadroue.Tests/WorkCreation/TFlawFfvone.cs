...
public sealed class TFlawFfvone
{
    private const string TFlawFfvoneStream =
        "[STREAM]\ncodec_type=video\ncodec_name=ffv1\n[/STREAM]\n"
        + "[FORMAT]\nformat_name=matroska,webm\n[/FORMAT]\n";

    private const string TFlawAvcStream =
        "[STREAM]\ncodec_type=video\ncodec_name=h264\n[/STREAM]\n"
        + "[FORMAT]\nformat_name=matroska,webm\n[/FORMAT]\n";
    ...
    {
        Assert.Null(TInterface.TFlawFfvoneResolve(
            TFlawAvcStream,
    ...
    {
        Assert.Null(TInterface.TFlawFfvoneResolve(TFlawFfvoneStream, string.Empty));
    }

    [Fact]
    public void FfvoneCrcMismatch_ProducesReportOnlyDossier()
    {
        LDossier? dossier = TInterface.TFlawFfvoneResolve(
            TFlawFfvoneStream,
            "[ffv1 @ 0x1] CRC mismatch 1A2B3C4D!");
