...
public sealed class TFlawTruncation
{
    [Fact]
    public void CleanFile_ProducesNoTruncationDossier()
    {
        Assert.Null(TInterface.TFlawTruncationResolve(string.Empty, string.Empty));
    }

    [Fact]
    public void MissingFinalMetadata_RebuildsFinalization()
    {
        LDossier? dossier = TInterface.TFlawTruncationResolve(
