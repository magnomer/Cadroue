...
public sealed class TFlawTransport
{
    private const string TFlawTransportTs =
        "[STREAM]\ncodec_type=video\ncodec_name=h264\n[/STREAM]\n"
        + "[FORMAT]\nformat_name=mpegts\n[/FORMAT]\n";

    private const string TFlawTransportIso =
        "[STREAM]\ncodec_type=video\ncodec_name=h264\n[/STREAM]\n"
    ...
    {
        Assert.Null(TInterface.TFlawTransportResolve(
            TFlawTransportIso,
    ...
    {
        Assert.Null(TInterface.TFlawTransportResolve(TFlawTransportTs, string.Empty));
    }

    [Fact]
    public void ContinuityFault_ProducesTransportDossier()
    {
        LDossier? dossier = TInterface.TFlawTransportResolve(
            TFlawTransportTs,
            "[mpegts @ 0x1] Continuity check failed for pid 256 expected 3 got 5");
