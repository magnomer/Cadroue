...
public sealed class TSplitCreation
{
    private static readonly string TSplitSource = Path.Combine("media", "source.mov");

    [Fact]
    public void NoSource_ProducesNoWork()
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(null, TSplitSectionCreate(1, 2, "First"));
    ...
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(TSplitSource);
    ...
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(TSplitSource, TSplitSectionCreate(1, 2, "Hidden", hidden: true));
    ...
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(
            TSplitSource,
            TSplitSectionCreate(2, 2, "Empty"),
            TSplitSectionCreate(5, 3, "Reversed"),
            TSplitSectionCreate(6, 8, "Valid"));
    ...
    public void EveryValidVisibleSection_ProducesExactlyOneWorkItem()
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(
            TSplitSource,
            TSplitSectionCreate(0, 1, "First"),
            TSplitSectionCreate(1, 2, "Hidden", hidden: true),
            TSplitSectionCreate(2, 4, "Second"),
            TSplitSectionCreate(5, 5, "Empty"));
    ...
    {
        LWorkItem item = Assert.Single(TSplitCreate(TSplitSource, TSplitSectionCreate(1, 2, "Opening Scene")));
    ...
    {
        LWorkItem item = Assert.Single(TSplitCreate(TSplitSource, TSplitSectionCreate(1, 2, "   ")));
    ...
    {
        LWorkItem item = Assert.Single(TSplitCreate(TSplitSource, TSplitSectionCreate(1, 2, "Bad:Name?")));
    ...
    public void DuplicateGeneratedOutputNames_AreDisambiguated()
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(
            TSplitSource,
            TSplitSectionCreate(0, 1, "Repeated"),
            TSplitSectionCreate(1, 2, "Repeated"),
            TSplitSectionCreate(2, 3, "Repeated"));
    ...
    public void OutputExtensionAndDirectory_MatchRequestedExportSettings()
    {
        ...
        LEncoding output = TSplitOutputCreate(extension: ".mkv", folder: exportFolder);

        LWorkItem item = Assert.Single(TSplitCreate(TSplitSource, output, TSplitSectionCreate(1, 2, "Clip")));
    ...
    public void OneCreationRequest_AssignsSameNonemptyBatchIdentityToAllWork()
    {
        IReadOnlyList<LWorkItem> work = TSplitCreate(
            TSplitSource,
            TSplitSectionCreate(0, 1, "First"),
            TSplitSectionCreate(1, 2, "Second"));
    ...
    private static IReadOnlyList<LWorkItem> TSplitCreate(string? source, params LSplitSectionDescription[] sections) =>
        TSplitCreate(source, TSplitOutputCreate(), Guid.Empty, LWorkPriority.LWorkPriorityNormal, sections);

    private static IReadOnlyList<LWorkItem> TSplitCreate(
        string? source,
        LEncoding output,
        params LSplitSectionDescription[] sections) =>
        TSplitCreate(source, output, Guid.Empty, LWorkPriority.LWorkPriorityNormal, sections);
    ...
    private static IReadOnlyList<LWorkItem> TSplitCreate(
        ...
        Guid batchId,
        params LSplitSectionDescription[] sections) =>
        TSplitCreate(source, output, batchId, LWorkPriority.LWorkPriorityNormal, sections);
    ...
    private static IReadOnlyList<LWorkItem> TSplitCreate(
        ...
        params LSplitSectionDescription[] sections) =>
        TInterface.TSplitItemsCreate(
            priority,
            TInterface.TSplitDescriptionCreate(source, sections, output),
            "test-tab",
            _ => { },
            _ => { },
            batchId);
    ...
    private static LEncoding TSplitOutputCreate(string extension = "mp4", string? folder = null) =>
        TWorkOutput.TWorkSplitCreate(extension, folder);
}
