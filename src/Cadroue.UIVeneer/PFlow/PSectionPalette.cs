...
internal static partial class PSectionPalette
{
    ...
    {
        string pPaletteFolder = LDepot.LDepotPaletteRead();
        PSectionHiddenLoad(pPaletteFolder);
        pSectionLoaded.Clear();
        foreach (LSectionPaletteFile pFile in LSectionPalette.LSectionPaletteLoad(pPaletteFolder))
        {
            if (PSectionSwatchResolve(pFile) is not { } pPalette)
            {
                continue;
            }
