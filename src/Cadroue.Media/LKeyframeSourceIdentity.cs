...
{
    private const int LKeyframeSourceHashSampleSize = 1024 * 1024;
    ...
    public long LKeyframeSourceLastWriteUtcTicks { get; }

    public long LKeyframeSourceDurationMilliseconds { get; }

    public string LKeyframeSourcePartialHash { get; }
    ...
    private static string LKeyframeHashCreate(FileInfo fileInfo)
    {
        ...
        byte[] buffer = new byte[LKeyframeSourceHashSampleSize];
        ...
        if (stream.Length > LKeyframeSourceHashSampleSize)
        {
            stream.Position = Math.Max(0, stream.Length - LKeyframeSourceHashSampleSize);
    ...
    private string LKeyframeKeyCreate()
    {
        string rawKey = string.Join(
            ...
            LKeyframeSourceLastWriteUtcTicks,
            LKeyframeSourceDurationMilliseconds,
            LKeyframeSourcePartialHash);
